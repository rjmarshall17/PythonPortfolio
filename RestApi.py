...
ATMOS_HTTP_ERROR_CODES  = {
                           '400': {'bad request': {'atmos_error': '1056',
                                                   'description': 'The requested operation is not supported for symlinks.'}},
                           '401': {'unauthorized': {'atmos_error': '1015',
                                                    'description': 'The request could not be finished due to insufficient access privileges.'}},
                           '403': {'forbidden': {'atmos_error': '1033',
                                                 'description': 'Unable to retrieve the secret key for the specified user.'}},
                           '404': {'not found': {'atmos_error': '1003',
                                                 'description': 'The requested object was not found.'}},
                           '405': {'method not allowed': {'atmos_error': '1009',
                                                          'description': 'The method specified in the Request is not allowed for the resource identified.'}},
                           '409': {'conflict': {'atmos_error': '1006',
                                                'description': 'Operation aborted because of a conflicting operation in process against the resource.'}},
                           '416': {'requested range not satisfiable': {'atmos_error': '1004',
                                                                       'description': 'The specified range cannot be satisfied.'}},
                           '500': {'internal server error': {'atmos_error': '1040',
                                                             'description': 'The server is busy. Please try again'}}
                           }

class EsuRestApi(object):
 
    ID_EXTRACTOR = "/[0-9a-zA-Z]+/objects/([0-9a-f]{64})"
 
    def __init__(self, host, port, uid, secret,timeout=DEFAULT_TIMEOUT):
        """ Constructor that sets up the URL and appropriate credentials used to sign HTTP requests """
        
        self.host, self.port, self.uid, self.secret, self.timeout = host, port, uid, secret, timeout
    ...
    def create_object(self, data = "", user_acl = None, listable_meta = None, non_listable_meta = None, mime_type = None, checksum = None, group_acl = None):
        ...
        try:
            ...
            print "e.code=%s e.message=%s headers=%s" % (e.code,e.message,e.headers.headers)
            if e.code == 201:
    ...
    def create_object_on_path(self, path, user_acl = None, listable_meta = None, non_listable_meta = None, mime_type = None, data = "", group_acl = None, checksum=None):
        ...
        try:
            ...
            if response.code == 201:     
                return self.__parse_location(response)
            else:
                return response
    ...
    def get_system_metadata(self, object_id, sys_tags = None):                                                  
        ...
        try:
            ...
            if not SIMULATE:
                system_meta = []
                system_meta = response.info().getheader('x-emc-meta')
                system_meta = dict(u.split("=",1) for u in system_meta.replace(', ',',').split(","))     # Create a Python dictionary of the data in the header and return it.
                return system_meta
    ...
    def __parse_atmos_error(self, error):
        print error
        if re.search('<html>',error,re.MULTILINE):
            # This must be a ViPR error message
            res = re.search('<p>(.*)</p>',error,re.MULTILINE|re.DOTALL)
            http_error = re.search('<pre>\s*([^<]+)</pre>',res.group(1).replace('\n',''))
            if res:
                self.message = re.sub('<pre>\s*([^<]+)</pre>',r' \1',res.group(1).replace('\n',''))
            else:
                self.message = 'Unknown error'
            res = re.search('HTTP ERROR: (\d+)',error,re.MULTILINE|re.IGNORECASE)
            if res:
                if http_error is not None:
                    if ATMOS_HTTP_ERROR_CODES.has_key(res.group(1)) and ATMOS_HTTP_ERROR_CODES[res.group(1)].has_key(http_error.group(1).strip().lower()):
                        self.code = ATMOS_HTTP_ERROR_CODES[res.group(1)][http_error.group(1).strip().lower()]['atmos_error']
                else:
                    self.code = res.group(1)
            else:
                self.code = '-1'
        else:   
    ...
    def __parse_object_info_response(self, response):
        ...
        for object in tree.iter(NS + "GetObjectInfoResponse"):
            ...
            for subobject in object:
                object_tag = subobject.tag.split("}")[-1]
                object_dictionary[object_tag] = subobject.text.strip()
...
class EsuException(Exception):
    def __init__(self, http_code, atmos_error):
        ...
        self.atmos_error_code = atmos_error.code.text if hasattr(atmos_error.code,'text') else atmos_error.code
        self.atmos_error_message = atmos_error.message.text if hasattr(atmos_error.message,'text') else atmos_error.message
    def __str__(self):
        return repr(self)
