...
# The stop file is used to stop a long running job before it completes
DEFAULT_STOP_FILE = '/tmp/stop_create_test_files'
# If not exiting on errors, this is the maximum number of errors allowed before forcing an exit
# This is configurable via the command line
DEFAULT_MAX_ERRORS = 5
DEFAULT_TMP_DIRECTORY = '/tmp/create_test_files_%s' % datetime.now().strftime('%s')
# I currently do not have access to S3 or Swift, so these have not been tested
DEFAULT_S3_PORT = 0
DEFAULT_SWIFT_PORT = 0
...
# Convert a string in format: Nx, e.g. 10m for ten megabytes, to the integer equivalent
def human2bytes(n):
...
# Test files can be written with text to make errors easier to spot
def write_file(fname, size, log, lower=None, upper=None, add_content=None, buffer_size=human2bytes('128k')):
    ...
    count = 0
    for i in range(size // write_len):
        if write_buffer.tell() >= buffer_size:
            write_buffer.seek(0)
            data.write(write_buffer.read(buffer_size))
            count += 1
            remainder = write_buffer.read()
            log.debug("Current line number is %d, wrote buffer %d of size %d to data file, remainder: >>%s<<" % (
            i, count, buffer_size, remainder))
            write_buffer.close()
            write_buffer = StringIO()
    ...
    if total_written != size:
        raise RuntimeError(
            "Bytes written %s to file %s not equal to requested size: %s" % ('{:,}'.format(total_written),
                                                                             fname,
                                                                             '{:,}'.format(size)))
...
# Write a binary file using random data from os.urandom
def write_binary_file(fname, size, log, lower=0, upper=0, buffer_size=human2bytes('128k')):
    ...
    if size < buffer_size:
        ...
        data.write(os.urandom(size))
    else:
        count = 0
        while data_written < size:
            count += 1
            if (size - data_written) < buffer_size:
                data.write(os.urandom(size - data_written))
                log.debug("Wrote last buffer %d of size: %d to file data written: %d" % (
                    count, (size - data_written), data_written))
                data_written += (size - data_written)
            else:
                data.write(os.urandom(buffer_size))
    ...
    if total_written != size:
        raise RuntimeError("Bytes written %s to binary file %s not equal to requested size: %s" % (fname,
                                                                                                             '{:,}'.format(
                                                                                                                 total_written),
                                                                                                             '{:,}'.format(
                                                                                                                 size)))
    log.info("Wrote binary file %s with size: %d" % (fname, size))
    return size
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--format', help='The filename format for created files', dest='format',
    ...
    parser.add_argument('-c', '--count', help='The number of files to create in the bucket', dest='count', default=100,
                        type=int)
    ...
    parser.add_argument('-b', '--bucket',
                        help="The bucket to use for S3/Swift writes, will be created if it doesn't exist",
    ...
    parser.add_argument('-sp', '--swift-port', help='The Swift port, default=%d' % DEFAULT_SWIFT_PORT,
                        default=DEFAULT_SWIFT_PORT)
    parser.add_argument('-t', '--tmpdir', help='Temporary directory to use for large objects', dest='tmpdir',
                        default=DEFAULT_TMP_DIRECTORY)
    parser.add_argument('-k', '--keep-tmpdir', help='Keep the temporary director for objects', dest='keep',
    ...
    parser.add_argument('--no-clear-path', help='Answer to clear existing path is: No', dest='clear_path', default=True,
                        action='store_false')
    parser.add_argument('-v', '--verbose', help='Show status messages', dest='verbose', default=False,
                        action='store_true')
    parser.add_argument('--summary', help='Print a summary of files and bytes written when done', dest='summary',
                        default=False, action='store_true')
    parser.add_argument('--stop-file', help='The stop file to cleanly stop the creates', dest='stop_file',
                        default=DEFAULT_STOP_FILE)
    parser.add_argument('--no-exit-on-error', help='Do not exit after encountering an error on file',
                        dest='exit_on_error', default=True,
                        action='store_false')
    parser.add_argument('-m', '--max-errors',
                        help='If not exiting on error max errors in a row before exiting,'
                        'default=%d' % DEFAULT_MAX_ERRORS,
                        dest='max_errors', default=DEFAULT_MAX_ERRORS, type=int)
    parser.add_argument('-bf', '--binary-files', dest='binary_files', help='Write files a binary instead of text',
                        default=False, action='store_true')
    parser.add_argument('-l', '--log', help='Write log output to file', dest='log', default=None)
    parser.add_argument('--debug', dest='debug', help='Enable debug logging to stdout', default=False,
                        action='store_true')
    ...
    if args.bucket is None:
        if args.path != '.':
            if not os.path.isdir(args.path):
                ...
                ans = input("Path %s exists, clear it? (y/n) " % args.path)
        ...
        if args.secret is not None:
            ...
            try:
                ...
                log.debug("create_test_files.py: Hit exception (%s) %s when opening S3 bucket: %s" % (
            ...
            swift_client = swift.Connection(SWIFT_AUTH_URL_FMT.format(args.datanode, args.swift_port), args.user,
                                            args.swift_password)
            try:
        ...
        if not os.path.isdir(args.tmpdir):
            ...
            ans = input("Temporary directory %s exists, clear it? (y/n) " % args.tmpdir)
            if re.match('y(es)*', ans, re.IGNORECASE):
                rmtree(args.tmpdir)
                os.makedirs(args.tmpdir)
    ...
    if args.lower:
        if args.upper is None:
            raise ValueError("When specifying a lower range, an upper range is required")
    ...
    log.info("Starting test file creation to path: %s count=%d start=%d %s stop_file=%s" % (
    args.path, args.count, args.number, size_log, args.stop_file))
    ...
    for count in range(0, args.count):
        ...
        try:
            if args.bucket is None:
                if args.binary_files:
                    bytes_written = write_binary_file(args.format % (count + args.number), args.size, log, args.lower,
                                                      args.upper)
                else:
                ...
                log.debug("create_test_files.py: s3connection=%s s3bucket=%s swift_client=%s" % (
                    s3connection, s3bucket, swift_client))
                ...
                if s3connection is not None and s3bucket is not None:
                    if s3bucket.get_key(args.format % (count + args.number)) is None:
                        ...
                        log.debug("create_test_files.py: Wrote S3 data to bucket %s and key: %s" % (
                    ...
                    try:
                        ...
                        if e.http_reason == 'Not Found':
                            swift_etag = swift_client.put_object(args.bucket, objectname,
                                                                 open(args.format % (count + args.number), 'r').read(),
                                                                 headers={
                                                                     'Content-type': 'application/octet-stream'
                                                                     if args.binary_files else 'text/plain'
                                                                 }
                                                                 )
                            swift_object = swift_client.head_object(args.bucket, objectname)
        ...
        if count > 0 and count % verbose_count == 0:
            log.info("Test files created: %d test file: %s and bytes written: %s" % (
            count, args.format % (count + args.number),
            '{:,}'.format(total_written)))
        log.info("Files written: {0:,} wrote file: {1} bytes written: {2:,}".format(count,
                                                                                    args.format % (count + args.number),
                                                                                    total_written))
